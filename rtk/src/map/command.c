...
int command_url(USER* sd, char* line, lua_State* state) {
	...
	char url[128];
	int type = 0;
	if (sscanf(line, "%s %d %s", name, type, url) < 1)
		return -1;
...
int command_job(USER* sd, char* line, lua_State* state) {
	...
	if (SQL_ERROR == Sql_Query(sql_handle, "UPDATE `Character` SET `ChaPthId` = '%u', `ChaMark` = '%u' WHERE `ChaId` = '%u'", sd->status.class, sd->status.mark, sd->status.id))
	{

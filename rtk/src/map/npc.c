...
int warp_init() {
	...
	if (SQL_ERROR == SqlStmt_Prepare(stmt, "SELECT `SourceMapId`, `SourceX`, `SourceY`, `DestinationMapId`, `DestinationX`, `DestinationY` FROM `Warps`")
	...
	for (int j = 0; j < SqlStmt_NumRows(stmt) && SQL_SUCCESS == SqlStmt_NextRow(stmt); j++) {
		CALLOC(war, struct warp_list, 1);
		...
		war->tx = tx;
		war->ty = ty;
		if (mx > map[mm].xs - 1 || my > map[mm].ys - 1) printf("map id: %i, x: %i, y: %i,  source x or y out of bounds\n", mm, mx, my);
		if (tx > map[tm].xs - 1 || ty > map[tm].ys - 1) printf("map id: %i, x: %i, y: %i,  destination x or y out of bounds\n", tm, tx, ty);
...
int npc_init() {
	...
	if (SQL_ERROR == SqlStmt_Prepare(stmt, "SELECT `NpcId`, `NpcIdentifier`, `NpcDescription`, `NpcType`, `NpcMapId`, `NpcX`, `NpcY`, `NpcLook`, `NpcLookColor`, `NpcTimer`, `NpcSex`, `NpcSide`, `NpcState`,"
		"`NpcFace`, `NpcFaceColor`, `NpcHair`, `NpcHairColor`, `NpcSkinColor`, `NpcIsChar`, `NpcIsF1Npc`, `NpcIsRepairNpc`, `NpcIsShopNpc`, `NpcIsBankNpc`, `NpcReturnDistance`, `NpcMoveTime`, `NpcCanReceiveItem` FROM `NPCs%i` ORDER BY `NpcId`", serverid)
		|| SQL_ERROR == SqlStmt_Execute(stmt)
		...
		|| SQL_ERROR == SqlStmt_BindColumn(stmt, 11, SQLDT_UCHAR, &side, 0, NULL, NULL)
		|| SQL_ERROR == SqlStmt_BindColumn(stmt, 12, SQLDT_UCHAR, &state, 0, NULL, NULL)
		...
		|| SQL_ERROR == SqlStmt_BindColumn(stmt, 18, SQLDT_UCHAR, &npctype, 0, NULL, NULL)
		|| SQL_ERROR == SqlStmt_BindColumn(stmt, 19, SQLDT_UINT, &npcf1npc, 0, NULL, NULL)
		|| SQL_ERROR == SqlStmt_BindColumn(stmt, 20, SQLDT_UCHAR, &npcrepairnpc, 0, NULL, NULL)
		|| SQL_ERROR == SqlStmt_BindColumn(stmt, 21, SQLDT_UCHAR, &npcshopnpc, 0, NULL, NULL)
		|| SQL_ERROR == SqlStmt_BindColumn(stmt, 22, SQLDT_UCHAR, &npcbanknpc, 0, NULL, NULL)
		|| SQL_ERROR == SqlStmt_BindColumn(stmt, 23, SQLDT_UCHAR, &npcreturndistance, 0, NULL, NULL)
		|| SQL_ERROR == SqlStmt_BindColumn(stmt, 24, SQLDT_UINT, &movetime, 0, NULL, NULL)
		|| SQL_ERROR == SqlStmt_BindColumn(stmt, 25, SQLDT_UCHAR, &receiveItem, 0, NULL, NULL)) {
		SqlStmt_ShowDebug(stmt);
		SqlStmt_Free(stmt);
		return 0;
	}
	...
	for (x = 1; x <= count && SQL_SUCCESS == SqlStmt_NextRow(stmt); x++) {
		...
		if (npcf1npc == 1) {
			...
			if (nd == NULL) { CALLOC(nd, NPC, 1); }
			else { map_deliddb(&nd->bl); }
		}
		else if (nd == NULL) { CALLOC(nd, NPC, 1); }
		else {
			map_delblock(&nd->bl);
			map_deliddb(&nd->bl);
		}
		...
		if (m != nd->startm || xc != nd->startx || yc != nd->starty) npc_warp(nd, m, xc, yc);
	...
	for (x = NPC_START_NUM; x <= npc_id; x++) {
		...
		if (nd) {
			if (nd->npctype == 1) {
				if (SQL_ERROR == SqlStmt_Prepare(stmt, "SELECT '', `NeqLook`, '1', '0', '0', `NeqColor`, `NeqSlot` FROM `NPCEquipment%d` WHERE `NeqNpcId` = '%u' LIMIT 14", serverid, nd->id)
					|| SQL_ERROR == SqlStmt_Execute(stmt)

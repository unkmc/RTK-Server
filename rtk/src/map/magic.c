...
int magicdb_read() {
	...
	if (SQL_ERROR == SqlStmt_Prepare(stmt, "SELECT `SplId`, `SplDescription`, `SplIdentifier`, `SplType`, `SplQuestion`, `SplDispel`, `SplAether`, `SplMute`, `SplPthId`, `SplLevel`, `SplMark`, `SplCanFail`, `SplAlignment`, `SplTicker` FROM `Spells` WHERE `SplActive` = '1'")
		...
		|| SQL_ERROR == SqlStmt_BindColumn(stmt, 9, SQLDT_UCHAR, &level, 0, NULL, NULL)
		|| SQL_ERROR == SqlStmt_BindColumn(stmt, 10, SQLDT_UCHAR, &mark, 0, NULL, NULL)
		|| SQL_ERROR == SqlStmt_BindColumn(stmt, 11, SQLDT_UCHAR, &canfail, 0, NULL, NULL)
		|| SQL_ERROR == SqlStmt_BindColumn(stmt, 12, SQLDT_CHAR, &alignment, 0, NULL, NULL)
		|| SQL_ERROR == SqlStmt_BindColumn(stmt, 13, SQLDT_UCHAR, &ticker, 0, NULL, NULL))
	{
		SqlStmt_ShowDebug(stmt);
		SqlStmt_Free(stmt);
		return 0;

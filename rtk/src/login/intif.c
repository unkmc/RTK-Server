...
////////// UPDATE IP ADDRESS IN DATABASE SO WE MAY TRACK USERS WHILE THEY ARE PLAYING.... ADDED 06-29-2017 ////////////////////////
		...
		sprintf(ipaddress, "%u.%u.%u.%u", CONVIP(session[RFIFOW(fd, 2)]->client_addr.sin_addr.s_addr));
		...
		if (SQL_ERROR == Sql_Query(sql_handle, "UPDATE `Character` SET `ChaLastIP` = '%s' WHERE `ChaName` = '%s'", ipaddress, sd->name)) {

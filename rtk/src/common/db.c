...
static void db_free_add(DBMap_impl* db, DBNode node, DBNode* root)
{
	...
	if (db->free_count == db->free_max) { // No more space, expand free_list
		...
		REALLOC(db->free_list, struct db_free, db->free_max);
	}
	node->deleted = 1;
...
static DBIterator* db_obj_iterator(DBMap* self)
{
	...
	CALLOC(it, struct DBIterator_impl, 1);
 ...
 * @param self Interface of the database
 * @param key Key that identifies the entry
 * @param CALLOC Function used to CALLOC the data if the entry doesn't exist
 * @param args Extra arguments for CALLOC
 * @return Data of the entry
 * @protected
 * @see DBMap#vensure
 */
 ...
 * @param key Key that identifies the entry
 * @param CALLOC Function used to CALLOC the data if the entry doesn't exist
 * @param ... Extra arguments for CALLOC
 * @return Data of the entry
 * @protected
 * @see DBMap#vensure
 * @see DBMap#ensure
 ...
 * CALLOCd for compilers that don't support casting to unions.

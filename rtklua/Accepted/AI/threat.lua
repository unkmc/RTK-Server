threat = {
	calcHighestThreat = function(mob)
		...
		if (mob.target > 0 and targetFacing ~= nil and targetFacing.ID ~= mob.target and not mob:checkToObject(mob:getBlock(mob.target)) and (#e > 0 or #f > 0 or #g > 0 or #h > 0)) then
			-- or #i > 0 or #j > 0 or #k > 0 or #l > 0)) then
			if (#e > 0) then
				for a = 1, #e do
	...
	getThreatTable = function(mob)
		...
		table.sort(threat_amount, function(a, b)
			return a > b
		end)

food_bags = {
	...
	removeFromBag = function(player, amberType)
		...
		if bag.dura - amount <= 0 then
			...
			player:addItem("empty_food_bag", 1)
	...
	presentOptions = function(player)
		...
		if choice == "Add food to bag." then
			food_bags.addToBag(player, amberType)
		elseif choice == "Remove food from bag." then
			food_bags.removeFromBag(player, amberType)
		end
	end
}
...
empty_food_bag = {
	use = async(function(player)
		...
		local foodtypes = {"beef", "lean_beef", "rare_pork", "egg"}
		...
		if player:hasItem("empty_food_bag", 1) ~= true then
			return
		end
		...
		player:removeItem("empty_food_bag", 1)
...
beef_bag = {
	use = async(function(player)
		food_bags.presentOptions(player)
	end)
}

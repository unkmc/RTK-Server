metal_bags = {
	...
	removeFromBag = function(player, amberType)
		...
		if bag.dura - amount <= 0 then
			...
			player:addItem("empty_metal_bag", 1)
	...
	presentOptions = function(player)
		...
		local choices = {}
		if bag.dura ~= bag.maxDura then
			table.insert(choices, "Add metal to bag.")
		end
		if bag.dura ~= 0 then
			table.insert(choices, "Remove metal from bag.")
		end

		local choice = player:menuString("What would you like to do?", choices)

		if choice == "Add metal to bag." then
			metal_bags.addToBag(player, amberType)
		elseif choice == "Remove metal from bag." then
			metal_bags.removeFromBag(player, amberType)
		end
	end
}
...
empty_metal_bag = {
	use = async(function(player)
		...
		local metaltypes = {"metal", "fine_metal"}
		...
		for i = 0, 52 do
			...
			if item ~= nil then
				for j = 1, #metaltypes do
					if item.yname == metaltypes[j] then
						table.insert(foundMetal, item.yname)
		...
		if #foundMetal == 0 then
			player:dialogSeq(
				{
					t,
					"You must have some metal in your inventory to be able to add to the bag."
				},
				0
			)
			return
		...
		local choiceid = player:buy(
			"What type of metal do you want to add?",
			foundMetal,
			amounts,
			{},
			{},
		...
		if invItem ~= nil then
			...
			if invItem.amount > 1 then
				amount = player:inputNumberCheck(player:input("How many " .. invItem.name .. "s would you like to add to your metal bag?"))
		...
		if player:hasItem("empty_metal_bag", 1) ~= true then
			return
		end
		...
		player:removeItem("empty_metal_bag", 1)
...
metal_bag = {
	use = async(function(player)
		metal_bags.presentOptions(player)
	end)
}

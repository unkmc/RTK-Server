chill = {
	cast = function(player, target)
		...
		player:sendStatus()
		player:playSound(43)
		player:sendMinitext("You cast Chill.")
		player:sendMinitext("Calling Chill.")
		target:setDuration("chill", duration)
		...
		elseif (target.blType == BL_PC and player:canPK(target)) then
			target:sendMinitext(player.name .. " casts Chill on you.")
			target:calcStat()
		end
	end,

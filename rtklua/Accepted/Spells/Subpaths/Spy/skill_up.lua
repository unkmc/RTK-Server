skill_up_spy = {
	cast = function(player)
		...
		if (player:hasDuration("skill_up_spy") == true and player.rage == 6) then
			-- cast cunning 2
			...
			player:setAether("skill_up_spy", aethers)
			player:sendAnimation(anim)
			player:playSound(705)
			player:sendMinitext("[Skill up 2] You hone your skills.")
			player:sendAction(6, 35)
			player:sendStatus()
		elseif (player:hasDuration("skill_up_spy") == true and player.rage == 7) then
			-- cast cunning 3
			...
			player:sendAction(6, 35)
			player:sendStatus()
		elseif (player:hasDuration("skill_up_spy") == true and player.rage == 9) then
			-- cast cunning 4
			...
			player:sendAction(6, 35)
			player:sendStatus()
		elseif (player:hasDuration("skill_up_spy") == true and player.rage == 10) then
			-- cast cunning 5
			...
			player:sendAction(6, 35)
			player:sendStatus()
		elseif (player:hasDuration("skill_up_spy") == true and player.rage == 12) then
			...
			player:sendAction(6, 35)
			player:sendStatus()
		elseif (player:hasDuration("skill_up_spy") == false) then
			...
			player:sendAnimation(anim)
			player:playSound(705)
			player:setDuration("skill_up_spy", duration)
			player:setAether("skill_up_spy", aethers)

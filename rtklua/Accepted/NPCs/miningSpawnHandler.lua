MiningSpawnHandlerNpc = {
	action = function(npc)
		--(npc,map,{mob ids},{mob counts},timer)

		--MiningSpawnHandlerNpc.handleSpawn(npc, 1002, {71}, {100}, 240, 86, 88, 136, 162) -- squirrels and rabbits ?

		MiningSpawnHandlerNpc.handleSpawn(npc, 1103, {535}, {30}, 120)
	end,
	...
	handleSpawn = function(npc, map, mobs, maxMobs, timer, spawnMinX, spawnMinY, spawnMaxX, spawnMaxY)
		MiningSpawnHandlerNpc.deleteMob(
			npc,
			map,
			mobs,
			maxMobs,
		...
		MiningSpawnHandlerNpc.spawnMob(
			npc,
			map,
			mobs,
			maxMobs,
